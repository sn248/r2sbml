...
- **Compartments whose volume changes** are handled when the volume follows a rate rule: the volume joins the state vector and every species inside it gains the dilution term `-[S] * (dV/dt) / V`, so a concentration falls as its compartment grows even with no reaction. If the volume follows an *assignment* rule the volume itself is still correct, but the dilution term cannot be formed — that needs a symbolic time derivative of the rule — and `convertReactions()` warns.
